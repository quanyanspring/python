...
if __name__ == "__main__":
    ...
    # 账户循环sql
    xf_account_sql = """
        select * from lf_account where account_type = 1 and acc_no = {accountNo} order by acc_no;
        """
    ...
    # 账户流水sql
    xf_trans_sql = """
        select * from lf_account_log where acc_no = {accNo} and other_account_type = 22 and id > {id} order by id ;
        """

    # 商户流水sql
    merchant_trans_sql = """
            select * from t_merchant_transaction where out_trans_no in {orderNoList} and target_acc_no = {targetAccNo};
            """
    ...
    while account_flag:
        ...
        if account_count == 1:
            account_no = "XF-200521-30191"
        if account_count == 3:
            break

        # 查询账户信息
        col_list, account_info_list = execute_sql(xf_account_sql.format(accountNo="'" + account_no + "'"), "查询账户",
                                                  db_info[1],
                                                  db_info[2], 1000)
        ...
        for index, row in enumerate(account_info_list):
            ...
            while none:
                ...
                # 查询账户流水
                xf_col_list, xf_trans_list = execute_sql(
                    xf_trans_sql.format(accNo="'" + acc_no + "'", id=get_count()),
                    "查询内部账户流水",
                    db_info[1], db_info[2], 100)
                if xf_trans_list == None or len(xf_trans_list) < 1:
                    print("内部账户：{0},流水结果集为空，结束循环".format(acc_no))
                    break
                if len(xf_trans_list) < 100:
                    none = False
                ...
                # order_no item字典
                order_dict = {}
                for idx, item in enumerate(xf_trans_list):
                    order_no = item[2]
                    trans_amount = item[7]
                    id = item[0]
                    temp_order_no_list.append("'" + str(order_no) + "'")
                    str_order_no = ",".join(temp_order_no_list)
                ...
                str_order_no = "(" + str_order_no + ")"

                # 查询商户流水
                mer_col_lis, mer_trans_list = execute_sql(
                    merchant_trans_sql.format(orderNoList=str_order_no, targetAccNo="'" + acc_no + "'"),
                    "查询商户账户流水",
                    db_info[1], db_info[2], )

                if mer_trans_list == None or len(mer_trans_list) < 1:
                    print("内部账户：{0},流水为空".format(acc_no))
                else:
                    ...
                    if xf_count != mer_count:
                        ...
                        for i in range(len(xf_trans_list)):
                            mer_trans_list.append("'" + str(xf_trans_list[i][2]) + "'")
                            mer_order_no = ",".join(mer_trans_list)
                        # 如果当前账户用户流水比商户多，不再对比该账户
                        print("商户账户：{0},用户流水条数：{1},商户流水条数：{2},用户流水：{3},商户流水：{4}".format(acc_no, xf_count, mer_count,
                                                                                        xf_order_no, mer_order_no))
                        dif = list(set(temp_order_no_list) ^ set(mer_trans_list))
                        str_dif = ",".join(dif)
                    ...
                    for i in range(len(mer_trans_list)):
                        out_trans_no = mer_trans_list[i][3]
                        trans_amt = mer_trans_list[i][7]
                        ...
                        if out_trans_no in order_dict.keys():
                            ...
                            print("开始写入错误数据")
                            worksheet.write(get_sheet_count(), 0, out_trans_no)
                            worksheet.write(get_sheet_count(), 0, order_dict[out_trans_no])
                            set_sheet_count()
    ...
    # 保存文件
    workbook.save('/Users/admin/Desktop/大等式/用户流水和商户流水比较.xls')
