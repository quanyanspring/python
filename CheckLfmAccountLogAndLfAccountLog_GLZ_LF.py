...
if __name__ == "__main__":
    ...
    diff_sql_c = """
        select l.id, l.order_no, l.trans_amount,l.source from (select * from lfm_account_log where create_time >= {0} and create_time < {1} and account_type = '20' and (other_account_type = '3' or other_account like 'GLZ%') and trans_amount > 0) lm right join (
            select * from lf_account_log where create_time >={2} and create_time < {3} and acc_no like 'GLZ%' and (other_acc_no regexp 'FL|JL|CB|CZ|CP|FF|QYFF') = 1  and trans_amount > 0 and other_acc_no <> 'CB-QY') l on lm.order_no = l.order_no or concat(lm.order_no,'-02') = l.order_no where lm.order_no is null order by l.id asc limit {4},{5};
    """

    # lfm_account_log < lf_account_log
    diff_sql_c_count = """
        select count(1) from (select * from lfm_account_log where create_time >= {0} and create_time < {1} and account_type = '20' and (other_account_type = '3' or other_account like 'GLZ%') and trans_amount > 0) lm right join (
            select * from lf_account_log where create_time >={2} and create_time < {3} and acc_no like 'GLZ%' and (other_acc_no regexp 'FL|JL|CB|CZ|CP|FF|QYFF') = 1 and trans_amount > 0 and other_acc_no <> 'CB-QY') l on lm.order_no = l.order_no or concat(lm.order_no,'-02') = l.order_no where lm.order_no is null;
    """
    ...
    diff_sql_c_tm = """
        select la.order_no from lfm_account_log la where order_no in ({0}) and account_type = '20' and (other_account_type = '3' or other_account like 'GLZ%');
    """
    ##########################################################################################################################################################################################################################################
        ...
        for i in range(0, circle):
            ...
            for j in range(len(row_list)):
                ...
                if row[3] > str(0):
                    ...
                    # 1.0迁移的数据通过订单号查询
                    id_list_1.append(str(row[1]).replace('-02', ''))
